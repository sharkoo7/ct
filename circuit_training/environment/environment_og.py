...
def cost_info_function(
    ...
    boundry_weight: float = 5.0) -> Tuple[float, Dict[Text, float]]:
  ...
  if boundry_weight > 0.0:
    boundry = get_boundry(plc)
    proxy_cost += boundry
...
def get_boundry(
    plc: plc_client.PlacementCost) -> float:
  ...
  for module_idx in macro_indices:
    # extract module information
    module_w,module_h = plc.get_node_width_height(module_idx)
    module_x, module_y = plc.get_node_location(module_idx)
    ur_x = max(ur_x,module_x + module_w / 2)
    ur_y = max(ur_y,module_y + module_h / 2)
    bl_x = min(bl_x,module_x - module_w / 2)
    bl_y = min(bl_y,module_y - module_h / 2)
  width = ur_x - bl_x
  height = ur_y - bl_y
  cost = (width * height) / (canvas_w * canvas_h)
  return cost
