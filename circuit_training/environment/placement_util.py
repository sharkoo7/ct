...
def restore_node_xy_coordinates(plc: plc_client.PlacementCost,
...
def fix_port_coordinates(plc: plc_client_os.PlacementCost) -> None:
...
def create_placement_cost(
    ...
    Args:
      ...
      os: if using open source plc_client.
    Returns:
...
def save_placement(plc: plc_client_os.PlacementCost, plc_file: str, pb_file: str = '',
...
def fd_placement_schedule(plc: plc_client_os.PlacementCost, pb_file: str, plc_file: str,
    ...
    plc_gg = create_placement_cost(pb_file, plc_file, os=False)
    ...
    use_sizes = False
    plc_gg.optimize_stdcells(use_current_loc, move_stdcells, move_macros,
                             log_scale_conns, use_sizes, io_factor, num_steps,
                             max_move_distance, attract_factor, repel_factor)
    for node_index in nodes_of_types(plc_gg, ['MACRO']):
        if plc_gg.is_node_soft_macro(node_index):
