...
def cost_info_function(
        plc: plc_client_os.PlacementCost,
        done: bool,
...
class CircuitEnv(object):
    ...
    def __init__(
            ...
            create_placement_cost_fn: Callable[
                ..., plc_client_os.PlacementCost] = placement_util.create_placement_cost,
            std_cell_placer_mode: Text = 'fd',
            cost_info_fn: Callable[[plc_client_os.PlacementCost, bool],
    ...
    """
        if not self._save_best_cost or (cost < self._saved_cost and
            ...
            if self._cd_finetune:
                self._run_cd()
                cost = self._cost_info_fn(plc=self._plc, done=True)[
                    0]  # pytype: disable=wrong-keyword-args  # trace-all-classes
    ...
    def place_node(self, node_index: int, action: int) -> None:
        shape = self._current_mask[action]
        self._plc.place_node(node_index, self.translate_to_original_canvas(action),shape)
    ...
    def analytical_placer(self) -> None:
        ...
        if self._std_cell_placer_mode == 'dreamplace':
            ...
            placement_util.save_placement(self._plc, placed_plc_file, placed_pb_file, True)
            placement_util.fd_placement_schedule(self._plc,placed_pb_file,placed_plc_file)
        else:
